// <auto-generated />
namespace NXUI;

using NXUI.HotReload.Metadata;
using NXUI.HotReload.Nodes;

/// <summary>
/// The avalonia builders.
/// </summary>
public static partial class Builders
{
    /// <summary>
    /// Creates a new instance of the <see cref="Avalonia.Controls.Window"/> class.
    /// </summary>
    /// <returns>The new instance of the <see cref="Avalonia.Controls.Window"/> class.</returns>
    public static WindowBuilder Window()
        => ElementBuilder.Create<Avalonia.Controls.Window>(TypeMetadata.Avalonia_Controls_Window, () => new Avalonia.Controls.Window());


    /// <summary>
    /// Creates a new instance of the <see cref="Avalonia.Controls.Window"/> class.
    /// </summary>
    /// <param name="ref">The reference of the <see cref="Avalonia.Controls.Window"/> instantiated class.</param>
    /// <returns>The new instance of the <see cref="Avalonia.Controls.Window"/> class.</returns>
    public static WindowBuilder Window(out ElementRef<Avalonia.Controls.Window> @ref)
    {
        return ElementBuilder.Create<Avalonia.Controls.Window>(TypeMetadata.Avalonia_Controls_Window, () => new Avalonia.Controls.Window())
            .WithRef(out @ref);
    }


    /// <summary>
    /// Creates a new instance of the <see cref="Avalonia.Controls.Window"/> class.
    /// </summary>
    /// <param name="impl">The impl value.</param>
    /// <returns>The new instance of the <see cref="Avalonia.Controls.Window"/> class.</returns>
    public static WindowBuilder Window(Avalonia.Platform.IWindowImpl impl)
        => ElementBuilder.Create<Avalonia.Controls.Window>(TypeMetadata.Avalonia_Controls_Window, () => new Avalonia.Controls.Window(impl));


    /// <summary>
    /// Creates a new instance of the <see cref="Avalonia.Controls.Window"/> class.
    /// </summary>
    /// <param name="ref">The reference of the <see cref="Avalonia.Controls.Window"/> instantiated class.</param>
    /// <param name="impl">The impl value.</param>
    /// <returns>The new instance of the <see cref="Avalonia.Controls.Window"/> class.</returns>
    public static WindowBuilder Window(out ElementRef<Avalonia.Controls.Window> @ref, Avalonia.Platform.IWindowImpl impl)
    {
        return ElementBuilder.Create<Avalonia.Controls.Window>(TypeMetadata.Avalonia_Controls_Window, () => new Avalonia.Controls.Window(impl))
            .WithRef(out @ref);
    }

}
